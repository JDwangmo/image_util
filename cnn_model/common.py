...
class CnnBaseClass(CommonModel):
    '''
        简介： CNN模型的公共父类,包含一些预定义的方法和变量。

        提示： 实现子类时，主要覆盖 create_network()函数
    ...
    def __init__(self,
        ...
        :type optimizers: str
        :param input_shape: cnn设置选项,输入图片的size，格式为： (通道数，图片高，图片宽).
        ...
        assert len(input_shape)==3, 'input_shape的维度一定是三维，格式为： (通道数，图片高，图片宽)！'
    ...
    def create_multi_size_convolution_layer(self,
        ...
        for items in convolution_filter_type:
            ...
            nb_filter, nb_row, nb_col, border_mode, k = items[:5]
        ...
        # if
        if K.ndim(conv_layers[0])==2:
            concat_axis = -1
        else:
            concat_axis = 2

        output = merge(conv_layers, mode='concat', concat_axis=concat_axis)
    ...
    def create_one_size_convolution_layer(
        ...
        if kwargs.get('flatten','none')!='none':
    ...
    def create_convolution_layer(
        ...
        if len(convolution_filter_type) == 0:
            ...
            # 需要进行卷积
            from keras import backend as K
            assert K.ndim(input_layer) == 4, '输入必须是4维！'
    ...
    def check_param(self,**kwargs):
        ...
        if kwargs.has_key('activation'):
            assert kwargs['activation'] in ['none', 'linear','relu','tanh'], 'create convolution layer error!activation 仅支持 none,linear,relu,tanh'
        ...
        if kwargs.has_key('flatten'):
            assert kwargs['flatten'] in ['none', 'flatten'], 'create convolution layer error!activation 仅支持 none,flatten'
    ...
    def create_network(self):
        ...
        # 1. 输入层
        model_input = Input((self.input_shape,), dtype='int32')
        # 2. Embedding层
        ...
        embedding_4_dim = Reshape((1, self.input_shape, self.word_embedding_dim))(embedding)
        # 5. 第一层多size卷积层（含1-max pooling），使用三种size.
