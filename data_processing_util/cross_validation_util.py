...
def data_split_k_fold(
    ...
    for x, y in zip(cross_validation_X_split.values(), cross_validation_y_split.values()):
        yield np.asarray(x), np.asarray(y,dtype=int)
...
def transform_cv_data(
    ...
    for dev_x,dev_y,val_x,val_y in cv_data:
        ...
        # print(dev_x[0])
        # print(val_x[0])
        # print(dev_x_features[0])
        # print(dev_x_features[-1])
        # print((dev_x_features[-1]*255+255).astype(dtype='int'))
        # print((dev_x_features[0]*255).astype(dtype='int'))
        # print(val_x_features[0])
        # import Image
        # im = Image.fromarray((dev_x_features[0]*255).astype(dtype='int'))
        # quit()
        # im.show()
        ...
        if kwargs.get('verbose',0)>1:
            if hasattr(feature_encoder,'vocabulary'):
            ...
            if hasattr(feature_encoder,'print_sentence_length_detail'):
